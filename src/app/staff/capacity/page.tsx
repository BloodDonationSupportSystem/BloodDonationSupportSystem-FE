...
export default function StaffCapacityPage() {
  ...
  const handleMultipleModalOk = async () => {
    try {
      ...
      if (selectedLocation) {
        const effectiveDate = values.dateRange[0].startOf('day');
        const expiryDate = values.dateRange[1].endOf('day');
  ...
  const handleOpenMultipleModal = () => {
    ...
    const monday = dayjs().startOf('week').add(1, 'day'); // Week starts on Sunday, so add 1 day for Monday
    const saturday = dayjs().startOf('week').add(6, 'day');
  ...
  return (
    ...
    >
      ...
      <Modal
        title="Add Multiple Capacity Slots"
      ...
      >
        ...
        >
          ...
          <Form.Item
            ...
            label="Notes"
          >
            <TextArea rows={3} placeholder="Optional notes about these time slots" />
          </Form.Item>
        </Form>
      </Modal>
    </StaffLayout>
